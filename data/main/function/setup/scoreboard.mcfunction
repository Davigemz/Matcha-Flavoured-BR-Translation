...
#Used to clear xp after a cetain time after interacting with an anvil (cannot use schedule, as only the server can run schedule
...
scoreboard players set 1 anvil_interaction 0
...
#On load, set the wandering trader timer, and reset ALL people who summoned him, becuase if we dont, functions that should be
#and itll never ever fix itself. So if the server crashes, or someone logs out whilst waiting, they will never have a wandering
